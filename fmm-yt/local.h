...
struct local {
	...
	void add(multipole& m)
	{
        ...
        for (int i = 0; i < 6; i++)
            for (int j = 0; j < 6; j++)
                values[i] += derivatives[i][j] * m.values[j];
	}
    ...
    glm::dvec2 calc(glm::dvec2 point, double m)
    {
        ...
        result.x += values[0];
        result.x += values[1] * (point.x - center.x);
        result.x += values[2] * (point.y - center.y);
        result.x += values[3] * 0.5 * (point.x - center.x) * (point.x - center.x);
        result.x += values[4] * (point.x - center.x) * (point.y - center.y);
        result.x += values[5] * 0.5 * (point.y - center.y) * (point.y - center.y);
